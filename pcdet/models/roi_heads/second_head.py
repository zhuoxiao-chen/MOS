...
class SECONDHead(RoIHeadTemplate):
    ...
    def forward(self, batch_dict):
        ...
        if cfg.get('TTA', None) and cfg.TTA.METHOD in ['memclr', 'iwa', 'mos', 'mos_online']:  # loss_rpn here is entropy loss
            batch_size = batch_dict['batch_size']

            batch_dict['shared_features']=shared_features
            targets_dict['shared_features']=shared_features
            # batch_dict['shared_features'] = torch.stack(torch.chunk(shared_features, batch_size))
            # targets_dict['shared_features'] = torch.stack(torch.chunk(shared_features, batch_size))
    ...
    def get_loss(self, tb_dict=None):
        ...
        if cfg.get('TTA', None) and cfg.TTA.METHOD == 'memclr':
            rcnn_loss = 0
            tb_dict['shared_features'] = self.forward_ret_dict['shared_features']

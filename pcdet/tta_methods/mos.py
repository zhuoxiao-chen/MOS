...
def mos(model, val_loader, rank, leave_pbar, ps_label_dir, cur_epoch, model_func=None, lr_scheduler=None,
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        if cfg.TTA.METHOD in ['iwa', 'avg_agg']:
        ...
        if cfg.TTA.METHOD in ['iwa', 'avg_agg'] and cfg.TTA.IWA_SETTING.AGGREGATE_START_CKPT <= samples_seen:
            ...
            if num_aggregate >= 3:
                ...
                if use_model_bank and samples_seen >= cfg.TTA.IWA_SETTING.BANK_START_CKPT:
                    ...
                    if latest_model is None:
                        ...
                        # c_hist = torch.zeros(cfg.TTA.IWA_SETTING.AGGREGATE_BANK)
                ...
                print("loading_time", loading_time - time.time())
                ...
                if cfg.TTA.METHOD == 'iwa':
                    ...
                    delta = 0.5 if not cfg.TTA.IWA_SETTING.get('BALANCE_SIMILARITY_FUN', None) else cfg.TTA.IWA_SETTING.BALANCE_SIMILARITY_FUN
                        ...
                        for j in range(len(feat_map_list)):
                            ...
                            for b in range(b_size):
                                if pred_box_list_whole_batch[j][b].shape[0] == 0 or pred_box_list_whole_batch[i][b].shape[0] == 0:
                                    # no box at all, we dont want it, make it to be similar to every model, thus assign very little weight
                            ...
                            # feat_sim = torch.nn.functional.cosine_similarity(feat_map_list[j], feat_map_list[i])
                            # feat_sim = torch.mean(feat_sim).item() * 0.5
                            ...
                            # torch.norm(torch.concat([feat_map_list[i].squeeze(), feat_map_list[j].squeeze()]), p='nuc')
                            #torch.linalg.matrix_rank(torch.concat([feat_map_list[i].squeeze(), feat_map_list[j].squeeze()]))
                    ...
                    # c = torch.ones(len(model_path_list)).cuda()

                    # for i in range(len(c)):
                    #     vis_pred_pc(target_batch, record_predicts[i][0], c = c[i].item())

                    if len(c) >=  cfg.TTA.IWA_SETTING.AGGREGATE_NUM:
                        if c_hist is None:
                            c_hist = torch.zeros(cfg.TTA.IWA_SETTING.AGGREGATE_NUM).cuda()
                        else:
                            c_hist = c_hist + c[:cfg.TTA.IWA_SETTING.AGGREGATE_NUM]

...
class KittiDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        """ For KITTI-C """
        self.kitti_c = True if dataset_cfg.get('CORRUPT') else False
        if self.kitti_c:
            import pathlib
            self.original_root_split_path = pathlib.Path(str(self.root_split_path).replace('KITTI-C', 'kitti'))
            self.corrupt_type = dataset_cfg.CORRUPT
            self.root_split_path = self.root_path / self.corrupt_type
        ...
        # if dataset_cfg.get('CORRUPT'):
        #     # self.mode = 'test'
        #     self.include_kitti_data(self.corrupt_type)
    ...
    def get_calib(self, idx):
        ...
        if self.kitti_c:
            calib_file = self.original_root_split_path / 'calib' / ('%s.txt' % idx)
        else:
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        eval_det_annos = copy.deepcopy(det_annos)[:32]
        eval_gt_annos = [copy.deepcopy(info['annos']) for info in self.kitti_infos][:32]
...
def vis_pc(data_dict, fg_color=[179, 179, 179]):
    ...
    # boxes = np.delete(data_dict['gt_boxes'], (0,2, 3,4, 7), axis=0)
    #
    # boxes = boxes + np.random.normal(0,0.1,boxes.shape)
    boxes = data_dict['gt_boxes']
    # boxes = boxes + np.random.normal(0, 0.1, boxes.shape)
    # boxes = boxes[1,:].reshape(1, 7)
    boxes = boxes[[3,4,5], :]
    boxes[0, :] = boxes[0, :] + np.random.normal(0, 0.1, boxes[1, :].shape)
    boxes[1,:] = boxes[1,:] + np.random.normal(0, 0.2, boxes[1,:].shape)
    boxes[2, :] = boxes[2, :] + np.random.normal(0, 0.1, boxes[1, :].shape)

    # boxes = np.concatenate((boxes, boxes[2].reshape(1,7)))
    # boxes[3, 1] = boxes[3, 1] +0.1
    # boxes[3, 0] = boxes[3, 0] + 4.5
    # boxes[3, :] = boxes[3, :] + np.random.normal(0, 0.2, boxes[1, :].shape)
    ...
    # print(data_dict['frame_id'], 'is done.')

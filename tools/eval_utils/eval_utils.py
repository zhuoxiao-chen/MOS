...
def eval_source_only_epoch(cfg, model, dataloader, epoch_id, logger, dist_test=False, save_to_file=False, result_dir=None, args=None):
    ...
    for i, batch_dict in enumerate(dataloader):
        ...
        id_to_pred_box[batch_dict['frame_id'][0]] = pred_dicts[0]['pred_boxes'].cpu().detach().numpy()
        if len(id_to_pred_box) >= 1000:
            with open('dpo_snow_car_pred_boxes_1k_high_threshold.npy', 'wb') as f:
                np.save(f, id_to_pred_box)

...
def main():
    ...
    if not args.eval_all:
        ...
        eval_output_dir = eval_output_dir / 'iter_all_default'
    ...
    # do not add disturb during eval
    if cfg.get("TTA", None) and cfg.TTA.get("DISTURBE_PC", None) and not cfg.TTA.DISTURBE_PC.EVAL_ENABLE:
        cfg.TTA.ENABLE = False

    if cfg.LOCAL_RANK == 0:
        # if cfg.DATA_CONFIG._BASE_CONFIG_.split('/')[-1].split('.')[0] == 'waymo_dataset':
        task_name = cfg.DATA_CONFIG.DATASET[0]+'2'+cfg.DATA_CONFIG_TAR.DATASET
        wandb.init(project='TTA_TEST_' + task_name, entity='uq-mm-team')

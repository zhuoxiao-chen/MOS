...
def train_model_st(model, optimizer, source_loader, target_loader, model_func, lr_scheduler, optim_cfg,
                   ...
                   max_ckpt_save_num=50, merge_all_iters_to_one_epoch=False, logger=None, ema_model=None,model_copy=None):
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True,
        ...
        for cur_epoch in tbar:
            ...
            """ Start Online TTA here! """
            import pcdet.tta_methods as tta
            tta_func = getattr(tta, cfg.TTA.METHOD)
            tta_func(
                model, target_loader, rank, model_func=model_func,
                lr_scheduler=cur_scheduler, accumulated_iter=accumulated_iter,
                optim_cfg=optim_cfg,
                tbar=tbar, tb_log=tb_log,
                total_it_each_epoch=total_it_each_epoch,
                dataloader_iter=dataloader_iter, ema_model=ema_model,
                leave_pbar=True, ps_label_dir=ps_label_dir,
                cur_epoch=cur_epoch, optimizer=optimizer,
                ckpt_save_interval_iter=cfg.TTA.SAVE_CKPT_INTERVAL,
                ckpt_save_dir=ckpt_save_dir,
                logger=logger,
                model_copy=model_copy
            )

            """ Online TTA ends here """
            exit()
            ...
            if (cur_epoch in cfg.SELF_TRAIN.UPDATE_PSEUDO_LABEL) or \
                ...
                self_training_utils.test_time_adaptation_one_epoch(
                    model, target_loader, rank,model_func=model_func,lr_scheduler=cur_scheduler,accumulated_iter=accumulated_iter, optim_cfg=optim_cfg,
                    tbar=tbar, tb_log=tb_log,total_it_each_epoch=total_it_each_epoch, dataloader_iter=dataloader_iter, ema_model=ema_model,
                    leave_pbar=True, ps_label_dir=ps_label_dir, cur_epoch=cur_epoch, optimizer=optimizer, ckpt_save_interval_iter=cfg.TTA.SAVE_CKPT_INTERVAL, ckpt_save_dir=ckpt_save_dir,
                    logger=logger
                )

...
class Detector3DTemplate(nn.Module):
    ...
    def load_params_from_file(self, filename, logger, report_logger = True, to_cpu=False):
        ...
        if version is not None and report_logger:
    ...
    def post_processing_multicriterion(self, batch_dict):
        ...
        for index in range(batch_size):
            ...
            if 'shared_features' in batch_dict.keys():
                pred_dicts[index]['shared_features'] = batch_dict['shared_features']
            if 'iou_penultimate_features' in batch_dict.keys():
                pred_dicts[index]['iou_penultimate_features'] = batch_dict['iou_penultimate_features']
            if 'spatial_features_2d' in batch_dict.keys():
                pred_dicts[index]['spatial_features_2d'] = batch_dict['spatial_features_2d']

        return pred_dicts, recall_dict

    def get_mem_loss(self, s_query, s_box_feat, mem_s_query, s_value,
                     t_box_feat, t_value, mem_bank, temperature=0.07,
        ...
        logits = torch.div(torch.matmul(anchor_feat, contrast_feat.T),
                           temperature)
        logits_max, _ = torch.max(logits, dim=1, keepdim=True)
        sm_logits = logits - logits_max.detach()

        mem_query = s_box_feat.mean(dim=[2, 3]).contiguous().unsqueeze(
            -1).unsqueeze(-1).permute(0, 2, 3, 1).detach()
    ...
    def mem_loss(self, t_batch, s_batch):
        ...
        pos_logits = torch.bmm(
            s_query.view(s_query.shape[0], 1, s_query.shape[1]),
            t_query.view(t_query.shape[0], t_query.shape[1], 1)).squeeze(-1)
        ...
        self.mem_bank = torch.cat(
            (mem_queue_t.t()[t_query.shape[0]:, :], t_query), dim=0).detach()

...
def main():
    ...
    results = '84.64 / 72.52 / 68.14,88.11 / 78.60 / 76.23,72.29 / 64.08 / 58.78,42.89 / 39.13 / 38.12 ,75.98 / 57.67 / 53.75,87.74 / 77.59 / 74.51,47.27 / 30.46 / 26.42,68.17 / 46.45 / 40.63'
    easy, mod, hard = [], [], []
    for r in results.split(','):
        easy.append(float(r.split('/')[0]))
        mod.append(float(r.split('/')[1]))
        hard.append(float(r.split('/')[2]))
    import numpy as np
    print(np.mean(easy), np.mean(mod), np.mean(hard))

    results = '67.64 / 27.48,78.96 / 59.20,82.19 / 61.83,65.09 / 30.12,67.46 / 35.34,65.81 / 30.39,78.67 / 56.52,83.29 / 73.45'
    bev, san_d = [],[]
    for r in results.split(','):
        bev.append(float(r.split('/')[0]))
        san_d.append(float(r.split('/')[1]))
    for b, s in zip(bev, san_d):
        print(100*(b-bev[0]) / (bev[-1] - bev[0]) , '/' ,100*(s-san_d[0]) / (san_d[-1] - san_d[0]) )
    ...
    if cfg.LOCAL_RANK == 0:
        wandb.init(project='TTA_train_' + cfg.DATA_CONFIG._BASE_CONFIG_.split('/')[-1].split('.')[0], entity='PUT_YOUR_ACCOUNT_HERE')
